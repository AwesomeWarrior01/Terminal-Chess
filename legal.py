...
# 4. Moving your own piece that would put your own king into chec
...
class Board:
    def __init__(self) -> None:
        with open('chess.csv', 'w') as board:
            ...
            # To edit the  starting position of the board, edit this
...
class Piece:
    ...
    def getLegal(self, pos, kingPos, piece, white):
        ...
        with open('piece.csv', 'w') as writer:
            ...
            loop = False
            if piece == 'q' or piece == 'Q': loop = True
            ...
            while True:
                ...
                if piece == 'n' or piece == 'N' or piece == 'k' or piece == 'K':
                    self.getLegal_limited(pos, kingPos, piece, white); break
                if piece == 'p' or piece == 'P':
                    self.getLegal_pawn(pos,white); break
                ...
                if self.enemyControlledSquare == 0:
                    ...
                    for k in range(6):
                        ...
                        if newPiece == self.whitePieces[k] and white == False:
                            self.legalMoves_general[result[0]][result[1]] = 'O'; break
    ...
    def getLegal_special(self, pos, kingPos, myPiece, lastEnemyPawnMove, numChecks):
        ...
        for i in range(8): self.legalMoves_special[i] = ['0','0','0','0','0','0','0','0']
        ...
        for i in range(4):
            for j in range(8):
                ...
                if piece_oldPos == diag_result:
                    ...
                    if test_piece != 'o': break
                else: break
            if super_break == True: break
    ...
    def getLegal_mate(self, kingPos, numChecks, white, pieceVectorTemp):
        ...
        if numChecks == 1:  
            ...
            if numMoves == 0: self.kingOnly = True
        # Need to make it so that only king moves are legal for more than one check.
        elif numChecks >= 2: self.kingOnly = True
    ...
    def getLegal_pieceControl(self, pos, kingPos, white, enemyPawn, pawnCapture):
                ...
                if white == True:
                    if piece == 'q' or (piece == 'b' and -1<i<4) or (piece == 'r' and 3<i<8):
                        ...
                        if pinnedState == [] or enemyPawn == True:
                            ...
                            if inCheck == 0: self.checkVector = checkVector
                            inCheck += 1
                            break
                        elif pinnedState != []: break
                    ...
                    # the check vector away from the check will count as a legal move when it actually shouldn't
                    elif piece == 'o' or piece == 'K': continue
                    else: break
        ...
        for l in range(2):
            ...
            if -1<result[0]<8 and -1<result[1]<8:
                ...
                or blocking2Move != 'o') and pawnCapture == False): break
                piece = self.get_piece(result)
    ...
    def getLegal_pawn(self, pos, white): 
        ...
        self.pawnRange = []
        # Here we check if there are available pieces to capture
        if white == True:
            ...
            else: capturePiece1 = 'o'
            if -1<capture2[0]<8 and -1<capture2[1]<8:
                capturePiece2 = self.get_piece(capture2)
            else: capturePiece2 ='o'
            for i in self.blackPieces:
                if capturePiece1 == i:      
                    self.pawnRange += [capture1]; break
            for i in self.blackPieces:
                if capturePiece2 == i:
                    self.pawnRange += [capture2]; break
